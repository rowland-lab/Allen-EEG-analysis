...
for sub=1:numel(sbj)
    ...
    % Load S1 Data
    s1_file=fullfile(wk_sbjfolder,'analysis','S1-VR_preproc',[sbj{sub},'_S1-VRdata_preprocessed_simple.mat']);
    if exist(s1_file)==0
        disp(['Preprocessing file is missing on ',sbj{sub}]);
        continue
    end
    s1_dat=load(fullfile(wk_sbjfolder,'analysis','S1-VR_preproc',[sbj{sub},'_S1-VRdata_preprocessed_simple.mat']));
    sessioninfo=s1_dat.sessioninfo;
    
    % Load S3 data
    s3_dat=load(fullfile(wk_sbjfolder,'analysis','S3-EEGanalysis','s3_dat_simple.mat'));
...
for sub=1:length(powerdat.data)
    ...
    for trial=1:length(fn)
        ...
        % Beta power contra
        powerdat.beta.contra{sub,1}(trialidx(trial),1)=mean(powerdat.data{sub}.(fn{trial}).atStartPosition.psd.saw(betafreq,contra_cn,:),'all','omitnan');
        powerdat.beta.contra{sub,2}(trialidx(trial),1)=mean(powerdat.data{sub}.(fn{trial}).cueEvent.psd.saw(betafreq,contra_cn,:),'all','omitnan');
        powerdat.beta.contra{sub,3}(trialidx(trial),1)=mean(powerdat.data{sub}.(fn{trial}).targetUp.psd.saw(betafreq,contra_cn,:),'all','omitnan');

        % Beta power ip
        powerdat.beta.ip{sub,1}(trialidx(trial),1)=mean(powerdat.data{sub}.(fn{trial}).atStartPosition.psd.saw(betafreq,ip_cn,:),'all','omitnan');
        powerdat.beta.ip{sub,2}(trialidx(trial),1)=mean(powerdat.data{sub}.(fn{trial}).cueEvent.psd.saw(betafreq,ip_cn,:),'all','omitnan');
        powerdat.beta.ip{sub,3}(trialidx(trial),1)=mean(powerdat.data{sub}.(fn{trial}).targetUp.psd.saw(betafreq,ip_cn,:),'all','omitnan');
    end
end
...
for d=1
    ...
    for ph=1:numel(phases)
        ...
        if norm
            if percent
                ylabel('% change in Beta Power normalized to baseline')
                ylim([-100 100])
            else
                ylabel('Change in Beta Power normalized to baseline')
                ylim([-1 1])
