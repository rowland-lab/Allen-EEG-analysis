...
if ~typecomp
    if ~isempty(EEG.icaact)
        ...
        % Reorganize Data into Event types (ALLEN)
        ...
        for et=1:numel(event_types)
            ...
            icaacttmp_reorder_new=cat(3,icaacttmp_reorder,icaacttmp(:,:,event_idx));
            icaacttmp_reorder=icaacttmp_reorder_new;
            trialeventnum(et)=sum(event_idx);
        end
        icaacttmp=icaacttmp_reorder;
    end
end
...
if EEG.trials > 1 % epoched data
    ...
    if typecomp == 1 % plot channel
        ...
        % ALLEN's EDIT --> add reorganized trial labels           
        set(axhndls{1},'YTick',[trialeventnum(1),sum(trialeventnum(1:2)),sum(trialeventnum(1:3))],'YTickLabel',{'Hold','Cue','Move'});
    end;
