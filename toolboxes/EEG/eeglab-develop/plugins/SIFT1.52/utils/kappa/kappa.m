...
default.values = {[],0,0.05,true};
default.values(1:nu) = args;
[x w alpha verb] = deal(default.values{:});
...
function kcomp
global m f x alpha verb po pe k sek ci km ratio kvar z p
...
a=Ex.*((f-wbar).^2);
kvar=(sum(a(:))-pe^2)/(n*((1-pe)^2)); %variance
z=k/sqrt(kvar); %normalized kappa
