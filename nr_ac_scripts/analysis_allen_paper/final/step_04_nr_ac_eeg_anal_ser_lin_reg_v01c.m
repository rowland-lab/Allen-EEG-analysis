function step_04_nr_ac_eeg_anal_ser_lin_reg_v01c(grp_stmstat,freq_band,mean_mean_sum_var,save_figs,save_data)

grp_stmstat='hc_non'
labl='vr'
epoch_type='vrevents'
freq_band='beta'
mean_mean_sum_var='~/nr_data_analysis/data_analyzed/eeg/gen_03/data/gen_03_ver_a_vr_mean_mean_beta_sum_hc.mat'
save_figs='yes'
save_data='yes'
ver='a'
    ...
    for k=1:2 %lat
        ...
        for l=freq_idx
            for o=1:3 %p
                ...
                eval(['annotation(''textbox'',[0.3323 0.9538 0.3409 0.0297],''String'',[''',grp_stmstat(1:2),' ',stim_status{j},' ',lat{k},' ',freq{l},' p',num2str(o),' corrected ',labl,'''],''FitBoxtoText'',''on'')'])
                if strcmp(save_figs,'yes')
                    %saveas(gcf,['~/nr_data_analysis/data_analyzed/eeg/gen_03/figures/gen_03_ver_',ver,'_corr_lr_',grp_stmstat,'_',lat{k},'_',freq{l},'_p',num2str(o),'.fig'],'fig')
                    saveas(gcf,['~/nr_data_analysis/data_analyzed/eeg/gen_03/figures/gen_03_ver_',ver,'_',labl,'_lr_corr_',grp_stmstat,'_',lat{k},'_',freq{l},'_p',num2str(o),'.fig'],'fig') 
                end
            end
        end
    end
...
if strcmp(save_data,'yes')
        save(['~/nr_data_analysis/data_analyzed/eeg/gen_03/data/gen_03_ver_',ver,'_',labl,'_lr_corr_',freq_band,'_',grp_stmstat]) 
end
