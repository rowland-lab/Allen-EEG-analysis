...
if strcmp(plot_ind,'yes')
    ...
    %             figure; set(gcf,'Position',[1119 25 744 898])
                ...
                eval(['title([''sbj',sbjname(15:16),':ch',num2str(l),':t',num2str(j),':p',num2str(i),':',freq_band,'''])'])
                ...
                for m=1:eval(['size(epochs.epochsWhole.t',num2str(j),'.',phase{i},'.val,1)'])%12 %plotting eeg signal for each reach
                    ...
                    title(['r',num2str(m)])
                    eval(['find_freq_',freq_band,'_t',num2str(j),'_p',num2str(i),'=find(epochs.epochsWhole.t',num2str(j),'.',phase{i},...
                        '.psd.freq>=',num2str(frq_rng_st),' & epochs.epochsWhole.t',num2str(j),'.',phase{i},'.psd.freq<=',num2str(frq_rng_ed),');'])
                    eval(['mean_',freq_band,'_t',num2str(j),'_p',num2str(i),'_c',num2str(l),'(',num2str(m),')=mean(log10(epochs.epochsWhole.t',num2str(j),...
                        '.',phase{i},'.psd.saw(find_freq_',freq_band,'_t',num2str(j),'_p',num2str(i),',',num2str(l),',',num2str(m),')))'])
                    eval(['[gen_var,out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),']=find(isoutlier(mean_',freq_band,'_t',num2str(j),'_p',...
                        num2str(i),'_c',num2str(l),',''quartiles'',''ThresholdFactor'',1.5))'])
                end

                subplot(4,4,[9 13]); hold on %plotting box plot
                eval(['boxplot(mean_',freq_band,'_t',num2str(j),'_p',num2str(i),'_c',num2str(l),')'])
                for n=1:eval(['size(epochs.epochsWhole.t',num2str(j),'.',phase{i},'.val,1)'])%12
                    eval(['text(1.1,mean_',freq_band,'_t',num2str(j),'_p',num2str(i),'_c',num2str(l),'(',num2str(n),'),''',num2str(n),''')'])
                end
            end
        end
    end
...
for l=[18 7]
    for j=1:4
        for i=1:3
            ...
            eval(['chk_outlier_var(',num2str(count),',4)=size(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),',2)'])
        end
    end
end
...
if strcmp(scroll,'yes')
    ...
    for l=[18 7]%i=1:3
        for j=1:4
            for i=1:3
                if eval(['isempty(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),')'])==1
                else
                    for k=1:eval(['size(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),',2)'])
                        eval(['epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                            '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                            '=epochs.epochsWhole.t',num2str(j),'.',phase{i},'.val(',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),',1)'])
                        eval(['epoch_ed_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                            '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                            '=epochs.epochsWhole.t',num2str(j),'.',phase{i},'.val(',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),',2)'])
            ...
            for i=1:3
                if eval(['isempty(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),')'])==1
                    ...
                    for k=1:eval(['size(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),',2)'])
                        if l==18
                            if i==1
                                eval(['plot(epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                        '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                        ':epoch_ed_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                        '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                        ',(trialData.eeg.data(epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                        '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                        ':epoch_ed_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                        '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                ...
                if eval(['isempty(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),')'])==1
                    ...
                    for k=1:eval(['size(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),',2)'])
                        if l==18
                            ...
                            eval(['set(gca(f1),''ylim'',[-25 -16],''xlim'',[epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                    '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                    '-20000 epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                    '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                    '+20000])'])
                        elseif l==7
                            input('press enter for next outlier')
                             eval(['set(gca(f1),''ylim'',[-6 6],''xlim'',[epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                    '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                    '-20000 epoch_st_out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),...
                                    '_',num2str(eval(['out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),'(',num2str(k),')'])),...
                                    '+20000])'])
...
for l=[18 7]%i=1:3
    for j=1:4
        ...
        for i=1:3
            count=count+1;
            if eval(['isempty(out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),')'])==1
                eval([sbjname,'_gen_03_ver_',version,'_reaches_wo_outliers_',freq_band,'{count}=[1:',num2str(eval(['size(epochs.epochsWhole.t',num2str(j),'.',phase{i},'.val,1)'])),'];'])
            else
                eval([sbjname,'_gen_03_ver_',version,'_reaches_wo_outliers_',freq_band,'{count}=setdiff([1:',num2str(eval(['size(epochs.epochsWhole.t',num2str(j),'.',phase{i},'.val,1);'])),...
                    '],out_mean_',freq_band,'_c',num2str(l),'_t',num2str(j),'_p',num2str(i),');'])
            end
        end
    end
end
...
if strcmp(filt,'no')
    if strcmp(save_outliers,'yes')
        save(['~/nr_data_analysis/data_analyzed/eeg/gen_03/data/',sbjname,'/analysis/S3-EEGanalysis/s3_dat.mat'],[sbjname,'_gen_03_ver_',version,'_reaches_wo_outliers_',freq_band],'-append')
    end
elseif strcmp(filt,'yes')
    if strcmp(save_outliers,'yes')
        save(['~/nr_data_analysis/data_analyzed/eeg/gen_03/data/',sbjname,'/analysis/S3-EEGanalysis/s3_dat_filt.mat'],[sbjname,'_gen_03_ver_',version,'_reaches_wo_outliers_',freq_band],'-append')
    end
end
