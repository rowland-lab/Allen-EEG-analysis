%function nr_ac_eeg_anal_ser_ind_freq_plots_v01d(sbjs,freq_band,epoch_type,version,date_str,save_fig,save_data)
...
%all of these en masse (may change my mind later)
...
%out_ind_c18_p0=[1 4 7 10];%we will call p0 rest
out_ind_c18_p1=[1 4 7 10];
out_ind_c18_p2=[2 5 8 11];
out_ind_c18_p3=[3 6 9 12];
%out_ind_c7_p0=[13 16 19 22];%these are backwards bc ch 18 was processed first
...
%         '/analysis/S3-EEGanalysis/s3_dat.mat'])
    ...
    figure; set(gcf,'Position',[2010 -22 754 891])
    for i=1:4
        subplot(10,5,i)
        hold on
        ...
        text(75,ylimasp(2)-0.1*ylimasp(2),num2str(eval(['size(epochs_rest.rest2.t',num2str(i),'.psd.saw,3)'])))
    ...
    mat_sz_plot_21{1}=linspace(0,0,10);
    ...
    mat_sz_plot_22{1}=linspace(0,0,10);
    ...
    mat_sz_plot_23{1}=linspace(0,0,10);
    ...
    mat_sz_plot_24{1}=linspace(0,0,10);
    ...
    mat_sz_plot_46{1}=linspace(0,0,10);
    ...
    mat_sz_plot_47{1}=linspace(0,0,10);
    ...
    mat_sz_plot_48{1}=linspace(0,0,10);
    ...
    mat_sz_plot_49{1}=linspace(0,0,10);
