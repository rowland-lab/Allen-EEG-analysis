function nr_ac_eeg_anal_ser_outlier_analysis_v01a(sbjfolder)
...
sp11=[2,3,4,6,7,8,10,11,12,14,15,16];
    ...
    %             figure; set(gcf,'Position',[1119 25 744 898])
                ...
                for m=1:eval(['size(epochs.vrevents.t',num2str(j),'.',phase{i},'.val,1)'])%12 %plotting eeg signal for each reach
                    subplot(4,4,sp(m))
...
%     for i=1:3
%         count=count+1;
%             eval(['chk_reach_count(',num2str(count),',1)=size(epochs.vrevents.t',num2str(j),'.',phase{i},'.val,1)'])
%     end
% end
% 
