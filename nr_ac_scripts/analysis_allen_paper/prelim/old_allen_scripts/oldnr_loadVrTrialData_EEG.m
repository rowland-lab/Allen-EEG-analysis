function [trialData,VR_chan_auto] = nr_loadVrTrialData_EEG(vrDataFolders,eegDataFile,eegSyncChannels,manualSyncReview,VR_chan)

% nr_loadVrTrialData_EEG(vrDataFolders,eegDataFile,{'DC1' 'DC2'},true); 
% 
% vrDataFolder=old_vrDataFolders

vrDataFolder=vrDataFolders;
...
if ~isempty(vrDataFolder)
    ...
    for it = 1:length(vrDataFolder)
        ...
        environmentFile_dir=dir(fullfile(vrDataFolder{it},'environment*'));
        environmentFile = fullfile(vrDataFolder{it},environmentFile_dir.name);
        if exist(vrDataFolder{it},'dir') && exist(trialInformationFile,'file') && exist(trackerDataFile,'file') && exist(analogDataFile,'file') && exist(digitalDataFile,'file') && exist(eventDataFile,'file')
            ...
            temp_env_dir=dir(fullfile(vrDataFolder{it},'environment*'));
            environment.type = extractBefore(temp_env_dir.name,'.xml');
...
if ~isempty(eegDataFile)
    ...
    if exist(eegDataFile,'file')
        [eegData, edfHdr] = nr_lab_read_edf(eegDataFile);
        eegData = eegData';
