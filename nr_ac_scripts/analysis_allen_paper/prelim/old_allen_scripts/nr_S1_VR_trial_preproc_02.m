function nr_S1_VR_trial_preproc_02(sbjnum,protocolfolder)
dbstop if error
...
% Load VR file
[trialData,vr_chan,tdcs_channels,Session_times] = nr_loadVrTrialData_EEG_03(vrDataFolders,eegDataFile,{'DC1','DC2'},{'DC3','DC4'},true); % Double-check that all your paths are added
%vr_chan=40;
...
for i=1:length(trialData.vr)
    ...
    if ~isfield(trialData.vr(i).events,'outsideStartPosition')
        continue
    else
        % Find VR events/sort
...
figure; hold on; set(gcf,'Position',[36 57 1415 871])
%('units','normalized','outerposition',[0 0 1 1]); hold on
plot((trialData.eeg.data(:,7)-mean(trialData.eeg.data(:,7)))/std(trialData.eeg.data(:,7))+20)
plot((trialData.eeg.data(:,18)-mean(trialData.eeg.data(:,18)))/std(trialData.eeg.data(:,18)))
plot((trialData.eeg.data(:,vr_chan)-mean(trialData.eeg.data(:,vr_chan)))/std(trialData.eeg.data(:,vr_chan))-20)
plot(((trialData.eeg.data(:,tdcs_chan)-mean(trialData.eeg.data(:,tdcs_chan)))/std(trialData.eeg.data(:,tdcs_chan))*-1)-40,'LineWidth',2)
...
[tdcs_detect,Session_times,VR_sig] = nr_tdcsdetect_02(trialData,vr_chan,tdcs_chan,vrDataFolders,Session_times);
