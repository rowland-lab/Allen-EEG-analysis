function [trialData,VR_chan_auto,tDCS_default,Session_positions] = loadVrTrialData_EEG_02(vrDataFolder,eegDataFile,eegSyncChannels,tDCSchannellabels,manualSyncReview,VR_chan)
...
if (vrLoaded && eegLoaded)
    ...
    if manualSyncReview
        ...
        if snr1>snr2
            plot(trialData.eeg.data(:,eegSyncChannels_idx{1}));
            ...
            if response==1
                ...
                plot(trialData.eeg.data(:,eegSyncChannels_idx{2}));
                title('Correct VR sync channels? [1=yes,2=no]')
                response2=input('Correct VR sync channels? [1=yes,2=no]');
                shg
                if response2==2
                    error('incorrect vr channel selection')
                else
